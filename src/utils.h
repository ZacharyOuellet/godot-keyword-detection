...
namespace Utils {
    ...
    void apply_preemphasis(std::vector<T>& values, const float coeff = 0.97f) {
        if (values.empty()) {
            return;
        }
    ...
    std::vector<float> dct(const std::vector<float>& values, const int num_coeffs) {
    ...
    void apply_mean_normalize(std::vector<float>& values) {
        float sum = 0.0f;
        for (float v : values) {
            sum += v;
        }
    ...
    void apply_fft(std::vector<std::complex<float>>& values, bool invert = false) {
        const int N = static_cast<int>(values.size());
        if (N == 1) return;
    ...
    template <typename T> // for float or std::complex<float>
    void apply_hann_window(std::vector<T>& frame) {
